/*
    filename:   vor.cpp
    contents:   output vorticity field by wavenumber
    make:       >g++ -fopenmp vor.cpp fft.o -o vor
...
void InitialState()
{
	...
	p = new IM[SIZE];
	vel[0] = Malloc2D(XN, YN, p);
	p = new IM[SIZE];
	vel[1] = Malloc2D(XN, YN, p);
	p = new IM[SIZE];
	vor = Malloc2D(XN, YN, p);
...
void vorticity_field( char *option, char *dstr )
{
...
#pragma omp parallel for
	...
	for (int i = 0; i < XN; i++ ) {
		for (int j = 0; j < YN; j++ ) {
			vel[0][i][j] = (wx0[i][j] + wx1[i][j]) / 2.0;
			vel[1][i][j] = (wy0[i][j] + wy1[i][j]) / 2.0;
		}
	}

	// wz = rot vz = d(vy)/dx - d(vx)/dy
	for (int i = 0; i < XN; i++) {
		for (int j = 0; j < YN; j++) {
			vor[i][j] = ((vel[0][i][(j+2)%YN]-vel[0][i][(j)%YN])/(2*DX))
				- ((vel[1][(i+2)%XN][j]-vel[1][(i)%XN][j])/(2*DY));
		}
	}

    fft2D(&vor[0][0], XN, YN, 1);
	for (int i=0;i < XN; i++) {
		double kx, ky;
		if (i < XN/2) {
			kx = dkx * i;
		...
		for (int j=0;j < YN; j++) {
            ...
            if ( !option_l.compare(option) ) {
                kkl = LKKL;
                kkh = LKKH;
	            sprintf(fname, "%s.vol", dstr);
            }
            else if ( !option_m.compare(option) ) {
                kkl = MKKL;
                kkh = MKKH;
	            sprintf(fname, "%s.vom", dstr);
            }
            else if ( !option_h.compare(option) ) {
                kkl = HKKL;
                kkh = HKKH;
	            sprintf(fname, "%s.voh", dstr);
            }
            else {
                kkl = LKKL;
                kkh = LKKH;
...
int main(int argc, char *argv[])
{
	...
	printf("\n");
	if ( ! argc == 2 ) {
		printf("usage :vor -l[,m,h] input-filename\n");
		exit(1);
	}
	vorticity_field( argv[1], argv[2]);
