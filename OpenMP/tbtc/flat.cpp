...
#pragma omp parallel for
	...
	IM max_wx0 = 0.0 + I * 0.0;
	IM max_wy0 = 0.0 + I * 0.0;
	IM max_wx1 = 0.0 + I * 0.0;
	IM max_wy1 = 0.0 + I * 0.0;
	for (int i=0;i < XN; i++) {
		for (int j=0;j < YN; j++) {
			if ( norm(max_wx0) < norm(wx0[i][j]) ) {
				max_wx0 = wx0[i][j];
			}
			if ( norm(max_wy0) < norm(wy0[i][j]) ) {
				max_wy0 = wy0[i][j];
			}
			if ( norm(max_wx1) < norm(wx1[i][j]) ) {
				max_wx1 = wx1[i][j];
			}
			if ( norm(max_wy1) < norm(wy1[i][j]) ) {
				max_wy1 = wy1[i][j];
			}
		}
	}
	for (int i = 0; i < XN; i++ ) {
		for (int j = 0; j < YN; j++) {
			wx0[i][j] = wx0[i][j] / max_wx0;
			wy0[i][j] = wy0[i][j] / max_wy0;
			wx1[i][j] = wx1[i][j] / max_wx1;
			wy1[i][j] = wy1[i][j] / max_wy1;
		}
	}
	...
	fclose(fp);
	printf( "\nFlatness velocity field process Done.\n" );
}
...
int main(int argc, char *argv[])
{
	...
	if ( outputfile == NULL ) {
		printf("can not open file : status.txt\n");
		exit(1);
	}
	t1 = time(NULL);
