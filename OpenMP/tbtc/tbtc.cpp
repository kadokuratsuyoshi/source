...
const double dwt = DT * 1000.0; // DT * 8000.0;
const double IR = 0.02;
const double TR = 100000;
const double f = 1.0 / (XN * YN);
const double kk = sqrt(AR * BR);
const double A0 = 1.0; // 1e-4; // 1.0; // 2.0; // 0.5; is stronger amplitude than 1.0 // 1.0;
const double kappa = 0.005; // 0.05; // 0.005; // 1.0; // 2.0 * 0.025; // 1.2; // 0.8; //  0.4; // 0.1; // 4.0 * 0.05; // large number to fast time to change
...
const IM GAMMA = 0.003 + I * 0.0; // 0.03 + I * 0.0; // 0.003 + I * 0.0; // 0.0003 + I * 0.0;
const double GG = 1.0;
const double G0 = 1.0;
const double G1 = 1.0;
...
void InitialState()
{
    ...
    p = new IM[XN*YN];
    Ck[0] = Malloc2D(XN, YN, p);
    p = new IM[XN*YN];
    Ck[1] = Malloc2D(XN, YN, p);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < XN; i++) {
        ...
        for (int j = 0; j < YN; j++) {
            double kvecy = j < YN / 2 ? j * dky : (j - YN) * dky;
			double ksh = sqrt( (kvecx * kvecx) + (kvecy * kvecy) );
			double kk = (2.0 * M_PI) / MAXNX;
            // kf = 1(kl=6,kh=8), kf = 2(kl=10,kh=12), kf = 3(kl=24,kh=36)
			double kl = kk * 18.0; // kk * 24.0; // 10.0; // 6.0; // 1.0;
			double kh = kk * 24.0; // 30.0; // kk * 36.0; // 12.0; // 8.0; // 4.0; // 8.0; // 3.0;
	        double Real0 = (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
	        double Imag0 = (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
	        double coff = sqrt(
                A0 * exp(
                    -(1.0/4.0) * delta * delta * (
	        ...
	        IM cd0 = Real0 * coff + I * Imag0 * coff;
			if ((kl < ksh) && (ksh < kh)) {
	        	Ck[0][i][j] *= f;
	        	Ck[0][i][j] += cd0 * DT - kappa * DT * Ck[0][i][j];
			}
			else {
	        	Ck[0][i][j] = 0.0 + I * 0.0;
			}
...
#pragma omp parallel for
    ...
    for (int i = 0; i < XN; i++) {
        ...
        for (int j = 0; j < YN; j++) {
            ...
            // kf = 1(kl=6,kh=8), kf = 2(kl=10,kh=12), kf = 3(kl=24,kh=36)
			double kl = kk * 18.0; // kk * 24.0; // 10.0; // 6.0; // 1.0;
			double kh = kk * 30.0; // kk * 36.0; // 12.0; // 8.0; // 4.0; // 8.0; // 3.0;
	        double Real1 = (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
	        double Imag1 = (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
	        double coff = sqrt(
                A0 * exp(
                    -(1.0/4.0) * delta * delta * (
                            (kvecx * kvecx) + (kvecy * kvecy)
	        ...
	        IM cd1 = Real1 * coff + I * Imag1 * coff;
			if ((kl < ksh) && (ksh < kh)) {
	        	Ck[1][i][j] *= f;
	        	Ck[1][i][j] += cd1 * DT - kappa * DT * Ck[1][i][j];
			}
			else {
	        	Ck[1][i][j] = 0.0 + I * 0.0;
			}
...
#pragma omp parallel for
    ...
    for (int i = 0; i < XN; i++) {
        for (int j = 0; j < YN; j++) {
            ...
            */
            cd0 += c0 * kk * (1/(sqrt(AR * BR))) * Ck[0][i][j];
            cd1 += c0 * kk * (1/(sqrt(AR * BR))) * Ck[1][i][j];
	        Psi[0][i][j] *= exp(cd0);
	        Psi[1][i][j] *= exp(cd1);
        }
    }
...
#pragma omp parallel for reduction (+:E_int)
    ...
    for (int i = 0; i < XN; i++) {
        for ( int j = 0; j < YN; j++) {
            ...
            double n_ii = G0 * d0 + G1 * d1;
            double n_ij = G01 * d0 + G01 * d1;
	        E_int += n_ii + n_ij;
        }
    }
    E_int *= DX * DY;
    ...
    if (argc == 2 ) {
        ...
        for (int i = 0; i < XN; i++) {
            for (int j = 0; j < YN; j++) {
	                ...
	                Psi[0][i][j] = re0 + I * im0;
                    Psi[1][i][j] = re1 + I * im1;
	                Ck[0][i][j] = reck0 + I * imck0;
	                Ck[1][i][j] = reck1 + I * imck1;
            }
            printf(".");
        }
        fin.close();
    	    ...
    	    if (t >= nextt) {
            	...
            	for (int i = 0; i < XN; i++) {
	            	...
	            	for (int j = 0; j < XN; j++) {
                    	...
                    	<< real(Ck[0][i][j]) << ' ' <<imag(Ck[0][i][j]) << ' '
                    	<< real(Ck[1][i][j]) << ' ' <<imag(Ck[1][i][j]) << ' '
                    	<< endl;
	            	}
                	out << endl;
            	}
