...
//void fft(complex<double> x[], int n, int flag);
//void fft2D(complex<double> x[], int n1, int n2, int flag);
//void fft3D(complex<double> x[], int n1, int n2, int n3, int flag);
...
*/
static void stockham(complex <double> x[], int n, int flag, int n2, complex <double> y[])
{
   complex <double>  *y_orig, *tmp;
   ...
   while(r >= n2) {                              /* loops log2(n/n2) times */
      ...
      for(j = 0; j < Ls; ++j) {
         ...
         for(k = jrs; k < jrs+r; ++k) {           /* "butterfly" operation */
            k2 = k + r;
            tr =  real(wr*y[k2]) - imag(wi*y[k2]);      /* complex multiply, w*y */
            ti =  imag(wr*y[k2]) + real(wi*y[k2]);
            //x[m].Re = y[k].Re + tr;
            //x[m].Im = y[k].Im + ti;
            x[m] = complex <double> (real(y[k])+tr, imag(y[k])+ti);
            //x[m2].Re = y[k].Re - tr;
            //x[m2].Im = y[k].Im - ti;
            x[m2] = complex <double> (real(y[k])-tr, imag(y[k])-ti);
            ++m;
            ++m2;
         }
      } 
...
*/
void cooley_tukey(complex <double> x[], int n, int flag, int n2)
{
   complex <double> c;
   int i, j, k, m, p, n1;
   int Ls, ks, ms, jm, dk;
      ...
      for(j = 0; j < Ls; ++j) {
         ...
         for(k = jm; k < n; k += dk) {                      /* "butterfly" */
            ...
            for(i = 0; i < n2; ++i) {                      /* for each row */
               ...
               tr =  real(wr*x[ms]) - imag(wi*x[ms]);
               ti =  imag(wr*x[ms]) + real(wi*x[ms]);
               //x[ms].Re = x[m].Re - tr;
               //x[ms].Im = x[m].Im - ti;
               x[ms] = complex <double>(real(x[m])-tr, imag(x[m])-ti);
               //x[m].Re += tr;
               //x[m].Im += ti;
               x[m] = complex <double>(real(x[m])+tr, imag(x[m])+ti);
            }
         }
         jm += n2;
      } 
...
void fft(complex <double> x[], int n, int flag)
{
   ...
   y = (complex <double> *) malloc( n*sizeof(complex <double>) );
   assert(NULL != y);
...
void fft2D(complex <double> x[], int n1, int n2, int flag)
{
   ...
   assert(1 == flag || -1 == flag);
   n = n1*n2;
   y = (complex <double> *) malloc( n2*sizeof(complex <double>) );
   assert(NULL != y);
...
void fft3D(complex <double> x[], int n1, int n2, int n3, int flag)
{
   ...
   n23 = n2*n3;
   n = n1*n23;
   y = (complex <double> *) malloc( n23*sizeof(complex <double>) );
   assert(NULL != y);
