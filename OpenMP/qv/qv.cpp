...
#pragma omp parallel for
    ...
    for (int i = 0; i < XN; i++) {
        for (int j = 0; j < YN; j++) {
			double trapx =((double)(i-(XN-1)/2.0)*DX) + CR * sin(OMEGA * t);
			double trapy =((double)(j-(YN-1)/2.0)*DY) + CR * -sin(OMEGA * t);
			double Rpo = (double) -0.5*trapx*trapx+-0.5*trapy*trapy;
