...
//const double A0 = 1.0e-2;
const double A0 = 1.0e0;
//const double kappa = 1.0e2;
const double kappa = 1.0e-1;
const double delta = ((1.41421356*XN)/(2.0*M_PI));
//const IM GAMMA = 1.0 + I * 0.0;
const IM GAMMA = 0.01 + I * 0.0;
#define XI (4)
const double GG = 1.0;
const double VE = 0.0;
const int TN = TR / DT;
	...
	for (int k = 0; k < ZN; k++ ) {
		for (int i = 0; i < XN; i++) {
			for (int j = 0; j < YN; j++) {
				...
				double Real = crand(); // (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
				double Imag = crand(); // (rand() % RAND_MAX) / ((double)RAND_MAX) - 0.5;
				Psi[i][j][k] = Real + I * Imag;
				//Psi[i][j][k] = 0.0 + I * Imag;
...
int main(int argc, char *argv[]){
	...
	if (argc == 2 ) {
		...
		for (int d = 1; 1; d++) {
			...
			Normalization();
			if(d % 10 == 0) {
				sprintf( str, "%.1f norm %.1f energy %.1f\n",
						it, norm(Psi[XN/2][YN/2][ZN/2]), CheckNormalization(), Energyfunc() );
				cerr << str;
				ofs << str;
			}
			if(it > IR) break;
...
#pragma omp parallel for
	...
	for (int d = 0; 1; d++) {
		...
		if ( readf != 0 ) {
			...
			sprintf( str, "%d system time %.1f norm %.1f energy %.1f\n",
					g, t, CheckNormalization(), Energyfunc() );
			cerr << str;
			ofs << str;
